...
NAME_UPDATED="false"
if [ "$ASSISTANT_NAME" != "TAi" ]; then
  log "Updating assistant name from TAi to $ASSISTANT_NAME"

  for md_file in groups/global/CLAUDE.md groups/main/CLAUDE.md; do
    if [ -f "$PROJECT_ROOT/$md_file" ]; then
      sed -i '' "s/^# TAi$/# $ASSISTANT_NAME/" "$PROJECT_ROOT/$md_file"
      sed -i '' "s/You are TAi/You are $ASSISTANT_NAME/g" "$PROJECT_ROOT/$md_file"
      log "Updated $md_file"
    else
